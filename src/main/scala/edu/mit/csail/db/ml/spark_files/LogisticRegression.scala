...
with HasStandardization with HasWeightCol with HasThreshold {
  ...
  override def toString(): String = {
    val featureString = $(featuresCol).mkString("[", ", ", "]")
 ...
 * in the future.
 * 
 * It is a smarter logistic regression which caches old models in the model database and looks them up before
...
class LogisticRegression(override val uid: String)
  ...
  /**
   * Segment train function into different parts to allow for optimization.
   * NOTE: This is a change from the original Spark LogisticRegression class.
   */
  ...
  def getInstances(dataset: DataFrame): RDD[Instance] = {
  ...
  def getSummarizers(instances: RDD[Instance]): (MultivariateOnlineSummarizer, MultiClassSummarizer) = {
  ...
  def runRegression(dataset: DataFrame,
    instances: RDD[Instance],
    summarizer: MultivariateOnlineSummarizer,
    labelSummarizer: MultiClassSummarizer,
    initialCoefficientsWithIntercept: Vector
