  ...
  override def update(oldModel: RandomForestClassificationModel,
    ...
    oldModel._trees.foreach(tree => {
      if (tree.weight < 0.1) {
        ...
        numReplacedTrees += 1
      } else if (treeSelector.nextFloat < incrementalProp) {
        incrementalTrees += tree
      } else if (incrementalProp < 1 && treeSelector.nextFloat() < 1 - regrowProp/(1-incrementalProp)) {
    ...
    val maxDepths = incrementalTrees.toArray.map(_.maxDepth + 1) ++
      Range(0,numReplacedTrees).map(_ => getMaxDepth)
    ...
    val updatedTrees = WahooRandomForest.runAndUpdateClassifier(
      ...
      maxDepths, oldModel.splits.get, oldModel.metadata.get)
      .map(_.asInstanceOf[DecisionTreeClassificationModel])

    println(numReplacedTrees + " trees replaced, " +
